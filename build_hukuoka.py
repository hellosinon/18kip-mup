...
def chrome() -> str:
  ...
  <div id="about">
    <div class="ab">
      ...
      <p>いまの時刻から、次に乗れる列車→次の駅→また次の列車、とつないだときの各駅到着を等時線で表したマップです。駅を選ぶと、その駅までの往路と、同じ所要で折り返したときの博多着（復路）も表示します。地図表現の都合上、主な路線および駅のみ掲載しています。</p>
      <h3>使い方</h3>
      <ul>
        <li>現在時刻を起点に、次列車の乗り継ぎで到達する駅を表示</li>
      ...
      <p class="cau">
        ※乗換待ちは始発パターンの間隔を流用した目安です。最終列車は考慮していません。
